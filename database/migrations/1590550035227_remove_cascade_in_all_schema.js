...
class RemoveCascadeInAll extends Schema {
  ...
  down() {
    this.table("users", (table) => {
      table.foreign("profile_id").references("profiles.id").onDelete("cascade").alter();
    });
    this.table("employees", (table) => {
      table.foreign("specialitie_id").references("specialities.id").onDelete("cascade").alter();
    });
    this.table("medical_schedules", (table) => {
      table.integer("patient_id").unsigned().notNullable().references("patients.id").onDelete("cascade").alter();
      table.integer("prescription_medicament_id").unsigned().references("prescription_medicaments.id").onDelete("cascade").alter();
